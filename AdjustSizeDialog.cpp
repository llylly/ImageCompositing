...
AdjustSizeDialog::AdjustSizeDialog(QWidget *parent): QDialog(parent) {
    this->width = 800;
    this->height = 600;
...
void AdjustSizeDialog::initRefresh(int width, int height) {
    this->width = width;
    this->height = height;
    this->available = false;
    this->widthEdit->setText(QString::number(this->width));
    this->heightEdit->setText(QString::number(this->height));
}

void AdjustSizeDialog::ok() {
    ...
    if (!(*legal)) {
        QMessageBox::information(this, "Adjust Size", "The height and width should be integers between 1 and 10000.", QMessageBox::Ok);
    } else {
        this->height = h;
        this->width = w;
        this->available = true;
